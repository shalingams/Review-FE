...
export default function Recipes() {
  const [recipes, setRecipes] = useState<TypeService[]>([]);
    ...
    <div>
      <section className="flex pt-20 pb-10 bg-[#F3F4F6] justify-center items-center">
        <div className="justify-center items-center w-3/4">
          <p className="text-4xl font-bold">Recipes</p>
          {loading && <p>Loading...</p>}
          {error && <p>Error</p>}
          <div className="flex flex-wrap -mx-4 mt-5">
            {recipes.map((recipe) => (
              <div key={recipe._id} className="w-full md:w-1/2 xl:w-1/3 px-4">
                <div className="bg-white rounded-lg overflow-hidden mb-10">
                  <img src={recipe.picture} alt="profile" className="w-full" />
                  <div className="p-8 sm:p-9 md:p-7 xl:p-9">
                    ...
                    <p className="flex text-base text-body-color leading-relaxed text-black">
                      ...
                      <span className="ml-2">
                        Start at € {recipe.availability[0].time.startTime}
                      </span>
                    </p>
